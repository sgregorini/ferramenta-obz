...
export default function Sidebar({ darkMode, setDarkMode, usuario }: SidebarProps) {
  ...
  return (
    <div className="w-64 h-full bg-[#F2F2F2] dark:bg-[#1F1F1F] text-[#212121] dark:text-white p-4 shadow-md flex flex-col justify-between transition-colors">
      {/* Logo / Title */}
      <div>
        ...
        <nav className="flex flex-col gap-1">
          {menu.filter(item => item.permitido).map(item => {
            ...
            return (
              <Link
                ...
                className={`flex items-center gap-3 px-4 py-2 rounded-lg text-sm font-medium transition-all
                  ${isActive ? "bg-brand-yellow text-black shadow-sm" : "hover:bg-[#E0E0E0] dark:hover:bg-[#333]"}`}
              >
                {item.icon}
      ...
      <div className="border-t pt-4 mt-6 flex items-center justify-between text-sm px-4">
        ...
        <button
          ...
          className="flex items-center gap-2 px-2 py-1 rounded hover:bg-[#E0E0E0] dark:hover:bg-[#333] transition"
          title="Alternar tema"
        >

...
export default function CadastroAtividadesNovaVisao({ usuario }: Props) {
  ...
  // --- NOVO: lista de IDs (próprio + subordinados) liberados ---
...
async function carregarAtividadesFiltradas() {
  ...
  // 1) Destructuring de data e error
  const { data, error } = await supabase
    .from("atividades")
    .select("*")
  ...
  // 2) Caso só um centro de custo
  if (centrosCustoSelecionados.length === 1) {
    setAtividades(data);
    return;
  }

  // 3) Monta o map de centro → lista de atividades
  const atividadesPorCentro: Record<string, Atividade[]> = {};
  centrosCustoSelecionados.forEach((cc) => {
    // Anotação de tipo em 'a' para não ficar any
    atividadesPorCentro[cc] = data.filter((a: Atividade) => a.centro_custo === cc);
  });

  // 4) Calcula a interseção
  const interseccao = atividadesPorCentro[centrosCustoSelecionados[0]].filter(
    (a: Atividade) =>
      centrosCustoSelecionados
        .slice(1)
        .every((cc) =>
          atividadesPorCentro[cc].some(
            (b: Atividade) => b.nome === a.nome && b.tipo === a.tipo
          )
        )
  );
...
async function getAllowedFuncionarioIds(funcionarioId: string): Promise<string[]> {
  ...
  if (error || !data) {
    ...
    return [funcionarioId]; // fallback: só o colaborador
  }

  // Remove duplicados se houver
  return Array.from(new Set(data.map((d) => d.id)));
}

useEffect(() => {
  (async () => {
    if (isAdmin) {
      // ADMIN: carrega todas as unidades de todos os funcionários
      const { data, error } = await supabase
        .from("funcionarios")
        .select("unidade")
        .not("unidade", "is", null);
  ...
  // carrega áreas: filtra por unidade + allowedFuncionarioIds
  useEffect(() => {
    (async () => {
      ...
      if (isAdmin) {
        ...
        if (!error && data) {
          areaIds = data.map((d) => d.area_id!).filter(Boolean);
        }
      ...
      setAreas(todasAreas.filter((a) => areaIds.includes(a.id)));
    })();
  }, [unidadeSelecionada, isAdmin, allowedFuncionarioIds]);

  // --- AQUI: carrega centros de custo com filtro de allowedFuncionarioIds ---
  useEffect(() => {
    (async () => {
      ...
      const centros = Array.from(new Set(data.map((d) => d.centro_custo))).sort();
      setCentrosCustoDisponiveis(centros);
    })();
  }, [unidadeSelecionada, areaSelecionada, isAdmin, allowedFuncionarioIds]);

  // carrega atividades após área + cc selecionados
  useEffect(() => {
    (async () => {
      if (!areaSelecionada || centrosCustoSelecionados.length === 0) {
        setAtividades([]);
      ...
      // interseção quando multicentro
      if (centrosCustoSelecionados.length === 1) {
        setAtividades(data);
      } else {
        const porCentro: Record<string, Atividade[]> = {};
        centrosCustoSelecionados.forEach((cc) => {
          porCentro[cc] = data.filter((a) => a.centro_custo === cc);
        });
  ...
  const handleSalvar = async () => {
    ...
    for (const cc of centrosCustoSelecionados) {
      ...
      const { error } = atividadeEditando
        ? await apiAdapter.updateAtividade(atividadeEditando.id, payload)
        : await apiAdapter.insertAtividade(payload);
      if (error) {
